...
function initButton() {
	...
	$("#btnAdd").click(function(event) {
		location.href="/etp/company/rAdminComPerUserAdd.do";
	});
	...
	$("#btnUpdate").click(function(event) {
		...
		if(sIDArray[0]!=null){
			window.location="/etp/company/rAdminComPerUpdate.do?uIds="+uIds;
		}else{
			alert("请选择一个用户");
		}
	});
	...
	$("#btnSubmit").click(function(event) {
		...
		var realName=$("#realName").val();
		var area = $("input[name='areaname']").val();
		var post = $("input:radio[name='post']:checked").val();
		$.ajax({
			type : "POST",
			url : "/etp/company/adminComPerUpdate.do",
			data : {uId:uId,area:area,realName:realName,post:post},
			dataType : 'json',
	...
	$("#btnSysMenu").click(function(event) {
		...
		var arrIDs = new Array();
			...
			var rec = jQuery("#listOperLog").jqGrid('getRowData', id);
			var vID = rec.ID;
			location.href="/etp/userMenu/rAdminComPerUserMenu.do?vID="+vID;
	});
}