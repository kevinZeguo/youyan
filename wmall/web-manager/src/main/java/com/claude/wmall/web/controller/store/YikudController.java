 ...
 */
@Controller
@RequestMapping("/store/yikud/")
public class YikudController {

    private static final Log log = LogFactory.getLog(YikudController.class);
    @Autowired
    private YikudService yikudService;
    @Autowired
    ...
    public View list(StoreYiKuD storeYiKuD) {
        PageInfo<StoreYiKuD> result = null;
        try {
        	result = this.yikudService.listByPage(storeYiKuD);
        } catch (Exception e) {
            log.error("zhuangCheCkdSerivce查询失败", e);
        }
        return new JqGrid2View<StoreYiKuD>(result);
    }
    ...
    public String del(String[]ids,HttpServletResponse response,HttpServletRequest request) throws Exception {
    	...
    	int result = yikudService.del(kundans);
    	HashMap<String,String> map=new HashMap<String,String> ();
    	System.out.println(result);
    	if(result==1){
    		map.put("result", "1");
    ...
    public View save(HttpServletRequest request, String data) {
        ...
        try {
            ...
            StoreYiKuD storeYiKuD = (StoreYiKuD) JSONObject.toBean(obj, StoreYiKuD.class, map);
            yikudService.save(storeYiKuD);
        } catch (Exception e) {
            log.error("保存入库单失败", e);
