 ...
 */
@Controller
@RequestMapping("/store/hongzixsckd/")
public class HongzixsCkdController {

    private static final Log log = LogFactory.getLog(HongzixsCkdController.class);
    @Autowired
    private HongzixsCkdService hongzixsCkdService;
    @Autowired
    ...
    public View list(StoreHongZiXsckd storeHongZiXsckd) {
        PageInfo<StoreHongZiXsckd> result = null;
        try {
        	result = this.hongzixsCkdService.listByPage(storeHongZiXsckd);
        } catch (Exception e) {
            log.error("zhuangCheCkdSerivce查询失败", e);
        }
        return new JqGrid2View<StoreHongZiXsckd>(result);
    }
    ...
    public String del(String[]ids,HttpServletResponse response,HttpServletRequest request) throws Exception {
    	...
    	int result = hongzixsCkdService.del(kundans);
    	HashMap<String,String> map=new HashMap<String,String> ();
    	System.out.println(result);
    	if(result==1){
    		map.put("result", "1");
    ...
    public View save(HttpServletRequest request, String data) {
        ...
        try {
            ...
            StoreHongZiXsckd storeYiKuD = (StoreHongZiXsckd) JSONObject.toBean(obj, StoreHongZiXsckd.class, map);
            hongzixsCkdService.save(storeYiKuD);
        } catch (Exception e) {
            log.error("保存入库单失败", e);
