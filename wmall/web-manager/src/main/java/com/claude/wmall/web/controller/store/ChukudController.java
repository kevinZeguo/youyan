 ...
 */
@Controller
@RequestMapping("/store/chukud/")
public class ChukudController {

    private static final Log log = LogFactory.getLog(ChukudController.class);
    @Autowired
    private ChukudService chukudService;
    @Autowired
    ...
    public View list(StoreChuKuD storeChuKuD) {
        PageInfo<StoreChuKuD> result = null;
        try {
        	result = this.chukudService.listByPage(storeChuKuD);
        } catch (Exception e) {
            log.error("zhuangCheCkdSerivce查询失败", e);
        }
        return new JqGrid2View<StoreChuKuD>(result);
    }
    ...
    public String del(String[]ids,HttpServletResponse response,HttpServletRequest request) throws Exception {
    	...
    	String [] kundans = ids[0].split("[,]");
    	int result = chukudService.del(kundans);
    	HashMap<String,String> map=new HashMap<String,String> ();
    	System.out.println(result);
    	if(result==1){
    		map.put("result", "1");
    ...
    public View save(HttpServletRequest request, String data) {
        ...
        try {
            ...
            StoreChuKuD storeChuKuD = (StoreChuKuD) JSONObject.toBean(obj, StoreChuKuD.class, map);
            chukudService.save(storeChuKuD);
        } catch (Exception e) {
            log.error("保存入库单失败", e);
